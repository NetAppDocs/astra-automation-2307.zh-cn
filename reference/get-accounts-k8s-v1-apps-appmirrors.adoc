...
summary: 列出应用程序的 AppMirror 资源 
---
= 列出应用程序的 AppMirror 资源
:allow-uri-read: 
...
| 个项 | 数组[任意类型：link:#appMirror_1_1_get_response_body["appMirror_1.1_get_response_body"]，数组[]] | True  a| 
...
| 角色 | string | False  a| 
此命名空间定义的初始角色。此参数是可选的，使用同一集群复制配置时除外。定义的值为：
...
| 角色 | string | False  a| 
此存储类定义的初始角色。此参数是可选的，使用同一集群复制配置时除外。定义的值为：
